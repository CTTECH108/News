...
export class AIService {
  ...
  async summarizeText(text: string, maxLength = 150): Promise<string> {
    try {
      ...
      const response = await openai.chat.completions.create({
        model: "deepseek/deepseek-r1", // Using DeepSeek R1 as specified
  ...
  async detectFakeNews(text: string, source?: string): Promise<{
    isReal: boolean;
    confidence: number;
    explanation: string;
    ...
    try {
      ...
      const trustedSources = ['thanthi', 'polimer', 'suntv', 'times of india', 'the hindu', 'indian express'];
      const sourceCredibility = source && trustedSources.some(trusted => 
        source.toLowerCase().includes(trusted)
      ) ? 'high' : 'medium';

      // If from trusted source, return real with high confidence
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are an expert fact-checker and misinformation detection specialist. Analyze news content for authenticity."
          },
          {
            role: "user",
            content: prompt
